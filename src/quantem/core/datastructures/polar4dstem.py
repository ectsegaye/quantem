...
def _amorphous_ring_model(
    ...
    params : torch.Tensor
        ...
        - x0, y0: center offsets from nominal center in pixels
    ...
    dx : torch.Tensor
        Horizontal pixel offsets from the nominal center.
    dy : torch.Tensor
        Vertical pixel offsets from the nominal center.

    Returns
    -------
    torch.Tensor
        Model intensity values at each (dx, dy) position.
    """
    ...
    x0, y0, a, b, theta = params[6], params[7], params[8], params[9], params[10]
    ...
    r_elliptical = torch.sqrt(torch.clamp((ux_rot / a) ** 2 + (uy_rot / b) ** 2, min=1e-12))

    # Central beam Gaussian (in pixel-radius space)
    r_pixels = torch.sqrt(torch.clamp(ux**2 + uy**2, min=1e-12))
...
def fit_elliptical_distortion(
    ...
    ----------
    data : Dataset4dstem
        4D-STEM dataset. The mean diffraction pattern is computed internally.
    center : (float, float)
    ...
    mask : ndarray, Tensor, or None
        Optional boolean mask (True = exclude pixel).
    device : str
        Torch device.
    max_iter : int
    ...
    yy = torch.arange(ny, dtype=torch.float32, device=device) - center_row
    xx = torch.arange(nx, dtype=torch.float32, device=device) - center_col
    dy_grid, dx_grid = torch.meshgrid(yy, xx, indexing="ij")
    r_grid = torch.sqrt(dx_grid**2 + dy_grid**2)
    ...
    dx_fit = dx_grid[annular_mask]
    dy_fit = dy_grid[annular_mask]
    ...
    if p0 is None:
        ...
        p0 = torch.tensor(
            [
                ...
                sigma2_init,
                c_bkgd_init,
                0.0,  # x0 offset (relative to provided center)
                0.0,  # y0 offset
    ...
    for _ in range(max_iter):
        ...
        model_vals = _amorphous_ring_model(phys, dx_fit, dy_fit)
    ...
    with torch.no_grad():
        ...
        final_loss = float(
            ((_amorphous_ring_model(final_params, dx_fit, dy_fit) - val_fit) ** 2).sum()
        )
    ...
    refined_row = center_row + float(final_params[7])
    refined_col = center_col + float(final_params[6])
