...
# ---------------------------------------------------------------------------
# Au FCC reference data
# ---------------------------------------------------------------------------
AU_LATTICE_PARAM = 4.0782  # Angstroms

# {hkl_label: h²+k²+l²}
AU_FCC_REFLECTIONS: dict[str, int] = {
    "111": 3,
    "200": 4,
    "220": 8,
    "311": 11,
...
def _au_q_values(a: float = AU_LATTICE_PARAM) -> dict[str, float]:
    """Return q = sqrt(h²+k²+l²) / a  for each Au FCC reflection."""
    return {hkl: np.sqrt(n) / a for hkl, n in AU_FCC_REFLECTIONS.items()}
...
class ReciprocalCalibration:
    ...
    def from_data(
        ...
        *,
        ...
        radial_profile = pdf.radial_mean.detach().cpu().numpy()
    ...
    def calibrate(
        ...
        ----------
        material
            Reference material.  Currently only ``'Au'`` is supported.
        peak_indices
    ...
    def plot_trial_pixel_sizes(
        ...
        """Overlay expected Au ring positions on the residual for several
